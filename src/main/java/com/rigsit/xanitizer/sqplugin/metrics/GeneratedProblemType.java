...
XANITIZER_PROBLEM_TYPE_5("LiteralStringsInVariablesSearch:password", "Hard-Coded Credentials in Variables: password", "Checks for password variables if they are assigned constant strings."),

XANITIZER_PROBLEM_TYPE_6("LiteralStringsInVariablesSearch:username", "Hard-Coded Credentials in Variables: username", "Checks for username variables if they assigned constant strings."),
...
XANITIZER_PROBLEM_TYPE_8("RegexSearch:PasswordInConfigFile", "Password in config file", "Checks for hard-coded passwords in config files.\n		  \n		  Hard-coded passwords in config files or other resource files are a risk factor."),
...
XANITIZER_PROBLEM_TYPE_10("SpecialMethodCall:java.util.Random", "'java.util.Random' Used in Project", "Instances of class 'java.util.Random' are not cryptographically secure. \n		   Consider instead using class 'java.security.SecureRandom' to get a cryptographically secure \n		   pseudo-random number generator for security-sensitive applications.\n		   \n		   See also CWE-330."),
...
XANITIZER_PROBLEM_TYPE_14("SpecialMethodCall:EncryptionUsedInProjectWOProvider", "Cryptographic Algorithms Used in Project, Provider Not Specified", "Searches in project code for all methods which set, select or determine a cryptographic algorithm and do not specify a \n		   crypto-provider for it, although an alternative method with a parameter for the provider is available.\n		   Not specifying the crypto-provider makes it uncertain which crypto-provider the server will use - it might be an unsafe one."),
...
XANITIZER_PROBLEM_TYPE_46("XPathSearch:sessionConfigWOHttpOnlyAndSecureInCookieConfig", "Application Server: cookie-config in web.xml: Not Properly Configured", "In web.xml it can be configured, in a cookie-config element nested in a session-config element, to prevent client side scripts from accessing the\n		   session ID and to force the browser to only use secure connections for transmission of the cookie.\n		   \n		   Xanitizer reports a finding if the web.xml cookie-config element is not configured in a way that both, the contained\n		   http-only flag and secure flag, are set to 'true'."),

XANITIZER_PROBLEM_TYPE_47("XPathSearch:sessionConfigWOTrackingModeCookieConfig", "Application Server: tracking-mode in web.xml: Not Set to 'COOKIE'", "In web.xml it can be configured, in a tracking-mode element nested in a session-config element, that \n		   the session IDs are only sent via cookies, not via request parameter.  \n		   \n		   Xanitizer reports a finding if there is\n		   no such configuration in a session-config element."),

XANITIZER_PROBLEM_TYPE_48("XPathSearch:sessionConfigMissing", "Application Server: session-config in web.xml Missing", "In web.xml, many security-relevant session properties should be configured in a session-config element; \n		   Xanitizer reports if this session-config element is not contained at all. A secure configuration looks like this:\n		   \n		   <web-app ...\n		   \n				<session-config>\n				\n				   <session-timeout>30</session-timeout>\n				   \n				   <cookie-config>\n				   \n				      <http-only>true</http-only>\n				\n				      <secure>true</secure>\n				      \n				   </cookie-config>\n				   \n				   <tracking-mode>COOKIE</tracking-mode>\n				   \n				</session-config>"),

XANITIZER_PROBLEM_TYPE_49("XPathSearch:exceptionTypeJavaLangThrowableMissing", "Application Server: error-page exception-type Configuration Wrong or Missing", "In web.xml, a catch-all error-page element should be defined that deals with all exceptions, i.e. is defined for exception-type java.lang.Throwable.\n		   This ensures that no internal error information is sent to the HTTP response by accident (where it could help a hacker to direct his attack.)\n		   A secure configuration looks like this:\n		   \n			  <error-page>\n			  \n			    <exception-type>java.lang.Exception</exception-type>\n			    \n			    <location>/error.jsp</location>\n			    \n			  </error-page>"),
